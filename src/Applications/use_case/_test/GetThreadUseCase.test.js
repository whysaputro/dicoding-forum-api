...
describe('GetThreadUseCase', () => {
  ...
  it('should _getRepliesForComment function operate correctly', () => {
    // Arrange
    const comments = [
      {
        id: 'comment-123',
    ...
    const expectedCommentsAndReplies = [
      ...
      { ...comments[1], replies: [{ ...filteredReplyB, content: '**balasan telah dihapus**' }] },
    ];

    const getThreadUseCase = new GetThreadUseCase({}, {}, {});

    const spyGetRepliesForComment = jest.spyOn(getThreadUseCase, '_getRepliesForComment');

    // Action
    getThreadUseCase._getRepliesForComment(comments, replies);

    // assert
    expect(spyGetRepliesForComment)
